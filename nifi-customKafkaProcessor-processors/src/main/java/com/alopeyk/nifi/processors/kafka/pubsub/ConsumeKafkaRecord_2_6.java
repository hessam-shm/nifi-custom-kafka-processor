...
import static com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.*;
...
@WritesAttributes({
    ...
    @WritesAttribute(attribute = com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_PARTITION, description = "The partition of the topic the records are from"),
    @WritesAttribute(attribute = com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_TIMESTAMP, description = "The timestamp of the message in the partition of the topic."),
    @WritesAttribute(attribute = com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_TOPIC, description = "The topic records are from")
})
...
@SeeAlso({com.alopeyk.nifi.processors.kafka.pubsub.ConsumeKafka_2_6.class, com.alopeyk.nifi.processors.kafka.pubsub.PublishKafka_2_6.class, com.alopeyk.nifi.processors.kafka.pubsub.PublishKafkaRecord_2_6.class})
public class ConsumeKafkaRecord_2_6 extends AbstractProcessor {
    ...
    static final PropertyDescriptor KEY_ATTRIBUTE_ENCODING = new Builder()
        ...
        .description("If the <Separate By Key> property is set to true, FlowFiles that are emitted have an attribute named '" + com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_KEY +
    ...
    private volatile com.alopeyk.nifi.processors.kafka.pubsub.ConsumerPool consumerPool = null;
    private final Set<com.alopeyk.nifi.processors.kafka.pubsub.ConsumerLease> activeLeases = Collections.synchronizedSet(new HashSet<>());

    static {
        List<PropertyDescriptor> descriptors = new ArrayList<>();
        descriptors.add(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.BOOTSTRAP_SERVERS);
        descriptors.add(TOPICS);
        descriptors.add(TOPIC_TYPE);
        descriptors.add(RECORD_READER);
        descriptors.add(RECORD_WRITER);
        descriptors.add(HONOR_TRANSACTIONS);
        descriptors.add(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.SECURITY_PROTOCOL);
        descriptors.add(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.SASL_MECHANISM);
        descriptors.add(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KERBEROS_CREDENTIALS_SERVICE);
        descriptors.add(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.JAAS_SERVICE_NAME);
        descriptors.add(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.USER_PRINCIPAL);
        descriptors.add(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.USER_KEYTAB);
        descriptors.add(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.USERNAME);
        descriptors.add(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.PASSWORD);
        descriptors.add(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.TOKEN_AUTH);
        descriptors.add(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.SSL_CONTEXT_SERVICE);
    ...
    @OnStopped
    public void close() {
        final com.alopeyk.nifi.processors.kafka.pubsub.ConsumerPool pool = consumerPool;
        consumerPool = null;
    ...
    protected PropertyDescriptor getSupportedDynamicPropertyDescriptor(final String propertyDescriptorName) {
        return new Builder()
                ...
                .addValidator(new com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KafkaConfigValidator(ConsumerConfig.class))
                .dynamic(true)
                .expressionLanguageSupported(ExpressionLanguageScope.VARIABLE_REGISTRY)
                .build();
    }
    ...
    protected Collection<ValidationResult> customValidate(final ValidationContext validationContext) {
        return com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.validateCommonProperties(validationContext);
    }

    private synchronized com.alopeyk.nifi.processors.kafka.pubsub.ConsumerPool getConsumerPool(final ProcessContext context) {
        com.alopeyk.nifi.processors.kafka.pubsub.ConsumerPool pool = consumerPool;
        if (pool != null) {
            return pool;
        }
    ...
    protected com.alopeyk.nifi.processors.kafka.pubsub.ConsumerPool createConsumerPool(final ProcessContext context, final ComponentLog log) {
        final int maxLeases = context.getMaxConcurrentTasks();
        final long maxUncommittedTime = context.getProperty(MAX_UNCOMMITTED_TIME).asTimePeriod(TimeUnit.MILLISECONDS);

        final Map<String, Object> props = new HashMap<>();
        com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.buildCommonKafkaProperties(context, ConsumerConfig.class, props);
        props.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "false");
        props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, ByteArrayDeserializer.class.getName());
        props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, ByteArrayDeserializer.class.getName());
        final String topicListing = context.getProperty(com.alopeyk.nifi.processors.kafka.pubsub.ConsumeKafkaRecord_2_6.TOPICS).evaluateAttributeExpressions().getValue();
        final String topicType = context.getProperty(com.alopeyk.nifi.processors.kafka.pubsub.ConsumeKafkaRecord_2_6.TOPIC_TYPE).evaluateAttributeExpressions().getValue();
        final List<String> topics = new ArrayList<>();
        final String securityProtocol = context.getProperty(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.SECURITY_PROTOCOL).getValue();
        final String bootstrapServers = context.getProperty(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.BOOTSTRAP_SERVERS).evaluateAttributeExpressions().getValue();
        ...
        if (topicType.equals(TOPIC_NAME.getValue())) {
            ...
            return new com.alopeyk.nifi.processors.kafka.pubsub.ConsumerPool(maxLeases, readerFactory, writerFactory, props, topics, maxUncommittedTime, securityProtocol,
            ...
            return new com.alopeyk.nifi.processors.kafka.pubsub.ConsumerPool(maxLeases, readerFactory, writerFactory, props, topicPattern, maxUncommittedTime, securityProtocol,
                bootstrapServers, log, honorTransactions, charset, headerNamePattern, separateByKey, keyEncoding);
        } else {
    ...
    public void interruptActiveThreads() {
        ...
        if (!activeLeases.isEmpty()) {
            int count = 0;
            for (final com.alopeyk.nifi.processors.kafka.pubsub.ConsumerLease lease : activeLeases) {
    ...
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {
        final com.alopeyk.nifi.processors.kafka.pubsub.ConsumerPool pool = getConsumerPool(context);
        if (pool == null) {
            context.yield();
            return;
        }

        try (final com.alopeyk.nifi.processors.kafka.pubsub.ConsumerLease lease = pool.obtainConsumer(session, context)) {
            if (lease == null) {
                context.yield();
                return;
            }
