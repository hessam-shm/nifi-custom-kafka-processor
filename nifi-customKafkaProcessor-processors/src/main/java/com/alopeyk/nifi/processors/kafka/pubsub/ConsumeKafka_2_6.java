...
import static com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.HEX_ENCODING;
import static com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.UTF8_ENCODING;
...
@WritesAttributes({
    @WritesAttribute(attribute = com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_COUNT, description = "The number of messages written if more than one"),
    @WritesAttribute(attribute = com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_KEY, description = "The key of message if present and if single message. "
            + "How the key is encoded depends on the value of the 'Key Attribute Encoding' property."),
    @WritesAttribute(attribute = com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_OFFSET, description = "The offset of the message in the partition of the topic."),
    ...
    @WritesAttribute(attribute = com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_PARTITION, description = "The partition of the topic the message or message bundle is from"),
    @WritesAttribute(attribute = com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_TOPIC, description = "The topic the message or message bundle is from")
})
...
public class ConsumeKafka_2_6 extends AbstractProcessor {
    ...
    static final PropertyDescriptor KEY_ATTRIBUTE_ENCODING = new PropertyDescriptor.Builder()
            ...
            .description("FlowFiles that are emitted have an attribute named '" + com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_KEY + "'. This property dictates how the value of the attribute should be encoded.")
            .required(true)
    ...
    static {
        ...
        descriptors.addAll(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.getCommonPropertyDescriptors());
    ...
    protected ConsumerPool createConsumerPool(final ProcessContext context, final ComponentLog log) {
        ...
        final byte[] demarcator = context.getProperty(com.alopeyk.nifi.processors.kafka.pubsub.ConsumeKafka_2_6.MESSAGE_DEMARCATOR).isSet()
                ? context.getProperty(com.alopeyk.nifi.processors.kafka.pubsub.ConsumeKafka_2_6.MESSAGE_DEMARCATOR).evaluateAttributeExpressions().getValue().getBytes(StandardCharsets.UTF_8)
                : null;
        ...
        final String topicListing = context.getProperty(com.alopeyk.nifi.processors.kafka.pubsub.ConsumeKafka_2_6.TOPICS).evaluateAttributeExpressions().getValue();
        final String topicType = context.getProperty(com.alopeyk.nifi.processors.kafka.pubsub.ConsumeKafka_2_6.TOPIC_TYPE).evaluateAttributeExpressions().getValue();
