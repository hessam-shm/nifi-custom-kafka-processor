...
import static com.alopeyk.nifi.processors.kafka.pubsub.ConsumeKafkaRecord_2_6.REL_PARSE_FAILURE;
import static com.alopeyk.nifi.processors.kafka.pubsub.ConsumeKafkaRecord_2_6.REL_SUCCESS;
...
public abstract class ConsumerLease implements Closeable, ConsumerRebalanceListener {
    ...
    private void processRecords(final ConsumerRecords<byte[], byte[]> records) {
        records.partitions().stream().forEach(partition -> {
            ...
            if (!messages.isEmpty()) {
                ...
                //our record is in pure form and no demarcator or factory
    ...
    private void writeData(final ProcessSession session, ConsumerRecord<byte[], byte[]> record, final TopicPartition topicPartition) {
        ...
        try {
            ...
            flowFile = session.putAttribute(flowFile, "messageContent", genericData.toString(record.value()));
            flowFile = session.putAttribute(flowFile, "messageContentRaw", record.value().toString());
            flowFile = session.putAttribute(flowFile, "messageContentArray", Arrays.toString(record.value()));
            flowFile = session.putAttribute(flowFile, "messageContentLength", Integer.toString(record.value().length));
    ...
    private void handleParseFailure(final ConsumerRecord<byte[], byte[]> consumerRecord, final ProcessSession session, final Exception cause, final String message) {
        ...
        attributes.put(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_OFFSET, String.valueOf(consumerRecord.offset()));
        attributes.put(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_TIMESTAMP, String.valueOf(consumerRecord.timestamp()));
        attributes.put(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_PARTITION, String.valueOf(consumerRecord.partition()));
        attributes.put(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_TOPIC, consumerRecord.topic());
        ...
        final String transitUri = com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.buildTransitURI(securityProtocol, bootstrapServers, consumerRecord.topic());
    ...
    private void populateAttributes(final BundleTracker tracker) {
        ...
        kafkaAttrs.put(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_OFFSET, String.valueOf(tracker.initialOffset));
        kafkaAttrs.put(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_TIMESTAMP, String.valueOf(tracker.initialTimestamp));
        ...
        if (tracker.key != null && (tracker.totalRecords == 1 || separateByKey)) {
            if (!keyEncoding.equalsIgnoreCase(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.DO_NOT_ADD_KEY_AS_ATTRIBUTE.getValue())) {
                kafkaAttrs.put(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_KEY, tracker.key);
            }
        }

        kafkaAttrs.put(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_PARTITION, String.valueOf(tracker.partition));
        kafkaAttrs.put(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_TOPIC, tracker.topic);
        if (tracker.totalRecords > 1) {
            ...
            if (tracker.recordWriter == null) {
                kafkaAttrs.put(com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.KAFKA_COUNT, String.valueOf(tracker.totalRecords));
            } else {
        ...
        final String transitUri = com.alopeyk.nifi.processors.kafka.pubsub.KafkaProcessorUtils.buildTransitURI(securityProtocol, bootstrapServers, tracker.topic);
